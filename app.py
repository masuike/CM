...
# UI構成
# =====================
st.title("🏗️ 施工管理AIツール (全機能フルスペック版)")
...
with col2:
    ...
    with tabs[2]:
        st.subheader("💎 案件マスター")
        master_area = st.text_area("直接編集・手入力", p_data["master_content"] if p_data else "", height=300)
        ...
        if chat := st.chat_input("マスターを踏まえて相談"):
    ...
    with tabs[3]:
        ...
        if acc_mode == "PDF/画像解析":
            ...
            if acc_f and st.button("🔎 教訓抽出"):
                raw = read_pdf_text(acc_f) if acc_f.type=="application/pdf" else "画像"
