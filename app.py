...
# UI構成
# =====================
st.title("🏗️ 施工管理AIツール (解析強化＆保存機能版)")
    ...
    with tabs[3]:
        ...
        if acc_mode == "解析":
            ...
            if st.button("登録"):
                ...
                st.rerun()
        with get_db() as conn:
            for r in conn.execute("SELECT * FROM accidents ORDER BY id DESC").fetchall(): st.error(r['content'])
