...
def analyze_vision(image, sys_p):
   ...
   res = client.chat.completions.create(
       ...
       messages=[{"role": "system", "content": sys_p},
                 {"role": "user", "content": [{"type":"text","text":"この手順書のリスクを精密に解析せよ。"},{"type":"image_url","image_url":{"url":f"data:image/png;base64,{img_str}"}}]}],
       temperature=0.0
   )
...
# UI構成
# =====================
st.title("🏗️ 施工管理AIツール (データ保存機能付き)")
       ...
       if os.path.exists(DB_PATH):
           with open(DB_PATH, "rb") as f:
               st.download_button(
                   label="📥 今のデータをPCに保存",
                   data=f,
                   file_name="construction_ai_backup.db",
                   mime="application/octet-stream",
                   help="これをダウンロードしておけば、消えても復活できます"
               )
...
with col2:
   ...
   with tabs[3]:
       ...
       acc_mode = st.radio("登録方法", ["PDF/画像解析", "テキスト入力"], horizontal=True)
       if acc_mode == "PDF/画像解析":
           acc_f = st.file_uploader("報告書をアップ", type=["pdf", "png", "jpg"], key="acc_up")
           if acc_f and st.button("🔎 教訓抽出"):
               raw = read_pdf_text(acc_f) if acc_f.type=="application/pdf" else "画像解析"
               lesson = ask_ai([{"role":"user", "content":f"事故報告から教訓を1行で抽出せよ: {raw}"}])
               with get_db() as conn:
